...
`注: 排查问题时发现，大部分情况都是enum导致的，有导出enum的文件如果导入过其他文件，就更有几率报错。直接把enum单独抽一个文件导出就能解决`
...
68. safari中，z-index不会跟随父级组件的z-index走 (跟视觉逻辑走，可以尝试用translateZ解决), 比如同时出现两个弹窗的时候, 就容易有bug，得单独为safari做兼容处理
...
81. safari14, 要求 sx + sWidth 和 sy + sHeight 不能大于源图像的宽高，否则 drawImage() 函数不会绘制任何图像 (但是源图的宽高判定又很混乱, 有时候canvas.width === sx + sWidth && canvas.height === sy + sHeight时也不生效, 难道是浏览器存着一份宽高且可以不为整数, 但是获取canvas宽高时为整数导致看起来是相等的)
